...
fun ColumnExample() {
  ...
  Column(
      modifier = Modifier.fillMaxSize().background(Color.Blue).padding(15.dp),
...
fun BoxExample() {
  ...
  Box(
      modifier = Modifier.size(200.dp).background(color = Color.Red),
      contentAlignment = Alignment.Center,
  ) {
    Box(modifier = Modifier.size(150.dp).background(color = Color.Black)) {}
  }
}

//constraint layout, use when it is needed and only for advance ui
@Composable
fun MyLayout() {
  ConstraintLayout() {
    Column(
      ...
      ConstraintLayout(
          modifier = Modifier.fillMaxWidth().height(100.dp).background(color = Color.LightGray)
      ) {
